...
# --- SOURCES ---
SOURCE_DSOJEVIC = "https://raw.githubusercontent.com/dsojevic/profanity-list/main/en.json"
...
def fetch_dsojevic() -> List[Dict]:
    ...
    try:
        ...
        } for item in data if item.get('match')]
...
def fetch_ldnoobw() -> List[Dict]:
    ...
    try:
        ...
        return [{"word": l.strip().lower(), "severity": "warning", "category": "profanity"} for l in lines if l.strip()]
    except Exception as e:
...
def fetch_hurtlex() -> List[Dict]:
    ...
    try:
        with urllib.request.urlopen(SOURCE_HURTLEX) as response:
            ...
            cat_map = {'PS': 'hate_speech', 'R': 'hate_speech', 'S': 'sexual', 'ASF': 'sexual', 'ASM': 'sexual', 'PR': 'sexual', 'OM': 'hate_speech', 'QAS': 'abuse', 'CDS': 'abuse'}
            for row in reader:
                word = row.get('lemma') or row.get('word')
                if word:
                    results.append({"word": word.lower().strip(), "severity": "warning", "category": cat_map.get(row.get('category'), 'profanity')})
        return results
    except Exception as e:
...
def seed_banned_words():
    ...
    def merge_list(source_list):
        for item in source_list:
            ...
            if w in all_words:
                ...
                if existing['category'] == 'profanity' and item['category'] != 'profanity':
    ...
    with engine.connect() as conn:
        ...
        for entry in final_list:
            ...
            count += 1
            if count % 500 == 0: logger.info(f"Processed {count}...")
        conn.commit()
        logger.info("Done!")
