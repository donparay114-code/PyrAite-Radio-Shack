...
async def test_warning_system_flow(async_client: AsyncClient):
    ...
    for i in range(1, 5):
        ...
        assert resp.status_code == 400, f"Expected 400 on attempt {i}, got {resp.status_code}: {resp.text}"
        data = resp.json()
        ...
        assert "free robux" in detail.get("flagged_terms", []) or "blocked terms" in detail.get("reason", "")

    # 3. Send 5th bad message -> Should cause Ban
    resp = await async_client.post(f"/api/chat/?user_id={user_id}", json=bad_message)
    assert resp.status_code == 403, f"Expected 403 Ban on 5th attempt, got {resp.status_code}: {resp.text}"
    ...
    assert resp.status_code == 403, "Banned user should not be able to send safe messages"
    assert "banned" in resp.json()["detail"].lower()
...
async def test_anon_immediate_rejection(async_client: AsyncClient):
    ...
    resp = await async_client.post("/api/chat/?anon_session_id=test_anon", json=bad_message)
